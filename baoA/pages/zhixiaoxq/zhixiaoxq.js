...
Page({
  ...
  getinfo() {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        if(res.data.result == 0){
          var tabindex;
          if (res.data.limitstate == 1){
            var newTime = new Date().getTime();
            var endTime = new Date(res.data.limitstarttime.replace(/-/g, "/")).getTime(); 
            var endTimed = new Date(res.data.limitendtime.replace(/-/g, "/")).getTime(); 
            if (endTime < newTime || endTimed < newTime) {
              var tabindex = 1;
              if (endTimed < newTime){
                _this.setData({
                  goodsistime:true
          ...
          var endTimeList = [];
          if (tabindex == 1) {
            var str = res.data.limitendtime.replace(/-/g, "/");
            endTimeList.push(str);

          } else if (tabindex == 2) {
            var str = res.data.limitstarttime.replace(/-/g, "/");
            endTimeList.push(str);

          }
          var info = res.data;
          var hpl = parseInt(res.data.shopScore * 100)
          wx.downloadFile({
      ...
      wx.request({
        ...
        success: function (res) {
          ...
          if (res.data.result == 0) {
            ...
            var now = _this.data.type1;
            wx.navigateTo({
              url: '../suredd/suredd?obj=' + obj + '&type=zhixiao&now='+ now,
            })
            _this.closechoosebox();
          } else {
            wx.showToast({
  ...
  onShareAppMessage: function (res) {
    ...
    return {
      path: "/pages/index/index?goodsid=" + goodsid + '&type=4&name=' + name + '&goodsimg=' + info.goodsImg[0] + '&userimg=' + userimg,
      imageUrl: shareimg,
      title: info.shopName
    };
  }
