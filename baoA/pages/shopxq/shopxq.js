...
Page({
  ...
  getshareimg() {
    ...
    // ctx.drawImage(imglist[0], 0, 0, 500, 350, 0, 0, 250, 150)
    // ctx.drawImage('../../images/index/xianshiimg.png',  150,0, 100,100)
    ...
    ctx.fillText("￥", 10, 185)
    ctx.setFontSize(22)
    ctx.fillText(String(info.goodsMinPrice), 24, 185)
    var moneywidth = ctx.measureText(String(info.goodsMinPrice)).width
    ...
    // ctx.fillText(discountText, moneywidth + 40, 178);
    // this.roundRect(ctx, moneywidth + 35, 164, boxWidth, boxHeight, bdRadius, bdBackground, bdColor)
    ctx.draw()
    setTimeout(function () {
      wx.canvasToTempFilePath({
        ...
        success(res) {
          ...
          //     console.log(res);
          //     var shareimg = res.data.imgUrllist[0];
          //     _this.setData({
          //       shareimg: shareimg
          //     })
          //   },
  ...
  getjuan(){
    ...
    wx.request({
      ...
      success: function (res) {
        console.log(res);
        _this.setData({
          juanlist: res.data.freeCouponList,
        })
  ...
  tanchusizebox(e) {
    ...
    if ((this.data.info.distribut == 1 || this.data.info.disstate == 1 || this.data.info.distribut == null) && e.currentTarget.dataset.type == 1){
      wx.showToast({
        title: '该商品暂不支持加入购物车',
        icon: 'none',
        duration: 1500
  ...
  onShareAppMessage: function (res) {
    ...
    return {
      path: "/pages/index/index?goodsid=" + goodsid + '&type=6&name=' + name + '&goodsimg=' + info.goodsImg[0] + '&userimg=' + userimg,
      imageUrl: shareimg,
      title: info.shopName
